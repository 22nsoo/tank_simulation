...
def pull_from_controller(base_url: str) -> dict[str, Any]:
    ...
    errors: list[str] = []
    try:
        raw = fetch_json(f"{base}/raw_lidar_status", timeout=10.0)
    except (OSError, URLError, json.JSONDecodeError) as exc:
        raw = {}
        errors.append(f"raw_lidar_status: {type(exc).__name__}: {exc}")
    try:
        analyzed = fetch_json(f"{base}/lidar_status", timeout=5.0)
    except (OSError, URLError, json.JSONDecodeError) as exc:
        analyzed = {}
        errors.append(f"lidar_status: {type(exc).__name__}: {exc}")
    return {"raw": raw, "analyzed": analyzed, "errors": errors}
...
def lidar_only_status():
    ...
    if source:
        ...
        source_errors = pulled["errors"]

    report = build_report(raw, analyzed, include_full=include_full)
    report["source"] = source or "local POST /info"
    report["sourceErrors"] = source_errors
...
<head>
  ...
  <title>LiDAR Information Visualizer</title>
  <style>
...
<body>
  <h1>LiDAR Information Visualizer</h1>
  <p>LiDAR 데이터 수신, 원시 정보 확인, 360도 포인트 클라우드 시각화 전용 서버입니다.</p>
  <div class="toolbar">
    <div>
      <input id="sourceUrl" value="" placeholder="선택: 다른 서버 주소 (예: http://127.0.0.1:5000)">
...
function renderDiagnosis(report) {
  ...
  document.getElementById('diagnosis').textContent = [
    `Source: ${report.source || 'local POST /info'}`,
    ...
    sourceErrors.length ? `Source error: ${sourceErrors.join(' | ')}` : 'Source connection: OK',
    '',
...
if __name__ == "__main__":
    port = int(os.getenv("LIDAR_VIEWER_PORT", "5051"))
    print(f"LiDAR visualizer: http://127.0.0.1:{port}/")
    print(f"LiDAR JSON      : http://127.0.0.1:{port}/status")
    print(f"LiDAR input     : POST http://127.0.0.1:{port}/info")
