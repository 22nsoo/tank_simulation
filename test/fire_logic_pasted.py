...
FAR_SPARSE_TANK_MIN_DISTANCE_M = 50.0
FAR_SPARSE_TANK_MIN_POINTS = 2
FAR_SPARSE_TANK_MIN_HEIGHT_SPAN_M = 0.45
...
BODY_ALIGN_TARGET_MAX_DISTANCE_M = 60.0
BODY_ALIGN_LOCK_RELEASE_DISTANCE_M = 68.0
...
BODY_ALIGN_SPATIAL_LOCK_ANGLE_DEG = 8.0
...
# ----------------------------------------------------------------------------
MODEL_PATH = Path(__file__).resolve().parent / "lalast.pt"
...
REQUIRE_YOLO_ENEMY_TANK_FOR_FIRE = True
YOLO_FIRE_DEBOUNCE_HITS = 1
YOLO_FIRE_DEBOUNCE_MISSES = 3
YOLO_REJECT_IGNORE_SECONDS = 9999.0
YOLO_FIRE_CENTER_X_TOLERANCE = 0.08
YOLO_FIRE_CENTER_Y_TOLERANCE = 0.22
YOLO_LIDAR_BIND_MAX_RESULT_AGE_SECONDS = 0.35
YOLO_LIDAR_BIND_MAX_TARGET_ANGLE_DRIFT_DEG = 6.0
YOLO_RECOGNITION_DELAY_AFTER_ALIGN_SECONDS = 0.35
YOLO_RECOGNITION_JUDGE_SECONDS = 1.0
YOLO_RECOGNITION_PREALIGN_DEG = 6.0
...
VISION_TURRET_WEIGHT_MIN = 0.04
VISION_TURRET_WEIGHT_MAX = 0.12
...
LIDAR_TURRET_YAW_WEIGHT_MIN = 0.035
LIDAR_TURRET_YAW_WEIGHT_MAX = 0.12
...
# ----------------------------------------------------------------------------
# Delay-compensated fine aiming
# ----------------------------------------------------------------------------
# The web/API state arrives a little late. Near the target, continuous commands
# cause overshoot: the turret passes the target, then comes back, then passes
# again. Do not delay Flask responses with sleep(). Instead, keep responding
# normally but send several STOP/no-input frames near the target so the simulator
# can settle and return a fresh state.
...
AIM_FINE_TURRET_WEIGHT_MIN = 0.015
AIM_FINE_TURRET_WEIGHT_MAX = 0.045
AIM_NEAR_BODY_LOCK_DEG = 6.0
AIM_BODY_LOCK_DISTANCE_M = 80.0

# Turret pulse aiming: instead of continuously rotating left/right, send one
# small yaw command, then intentionally rest for a short cooldown. This reduces
# oscillation when the web/API state is delayed.
TURRET_PULSE_AIM_ENABLED = True
TURRET_PULSE_COOLDOWN_SECONDS = 0.22
TURRET_PULSE_WEIGHT_FAR = 0.035
TURRET_PULSE_WEIGHT_CLOSE = 0.024
TURRET_PULSE_WEIGHT_FINE = 0.014
TURRET_LONG_RANGE_MICRO_DISTANCE_M = 60.0
TURRET_LONG_RANGE_PULSE_COOLDOWN_SECONDS = 1.00
TURRET_LONG_RANGE_WEIGHT_FAR = 0.010
TURRET_LONG_RANGE_WEIGHT_CLOSE = 0.007
TURRET_LONG_RANGE_WEIGHT_FINE = 0.004
TURRET_LONG_RANGE_STOP_DEADBAND_DEG = 0.25
TURRET_FINE_ZONE_DEG = 1.2
TURRET_CLOSE_ZONE_DEG = 3.0
TURRET_STOP_DEADBAND_DEG = 0.55

# Precision-aim response delay. This intentionally delays only the /get_action
# response near the target, not the LiDAR parsing/update routes. Keep these
# values small; too much sleep will make the whole controller feel late.
AIM_RESPONSE_SLEEP_ENABLED = True
AIM_FINE_RESPONSE_SLEEP_SECONDS = 0.16
AIM_CLOSE_RESPONSE_SLEEP_SECONDS = 0.10

# After a shot is emitted, temporarily ignore that LiDAR track so the controller
# can move to the next closest bulky/dangerous target instead of re-locking the
# same target while the simulator is still removing it.
ELIMINATED_TARGET_IGNORE_SECONDS = 1.75

# Automatic firing is disabled by default. The FIRE button only arms/approves
# a shot; /get_action fires only after the approved window overlaps strict aim
# readiness.
AUTO_FIRE_WHEN_STABLE = False
AUTO_FIRE_COOLDOWN_SECONDS = 0.85

# FIRE button activation should be looser than the actual firing gate.
# This lets the user press FIRE slightly before perfect alignment; the approval
# window then waits for the fine-aim stabilizer and strict yaw/pitch readiness.
FIRE_BUTTON_ENABLE_YAW_DEG = 2.2
FIRE_BUTTON_ENABLE_PITCH_DEG = 1.2
FIRE_BUTTON_APPROVAL_SECONDS = 3.0
# Manual fire behavior:
# The FIRE button should actually fire when the target is already close enough.
# Strict ready still exists for debug, but manual firing uses this looser gate so
# it does not get stuck at "Fire armed: waiting for fine aim" due to web/API
# latency, missing YOLO rawY, or a very tight distance-based yaw threshold.
...
latest_info_received_at = 0.0
detect_timing_history: deque[dict[str, Any]] = deque(maxlen=200)
...
yolo_fire_debounce_state: dict[str, Any] = {
    "targetKey": None,
    "trackId": None,
    "hits": 0,
    "misses": 0,
...
yolo_recognition_gate_state: dict[str, Any] = {
    "targetKey": None,
    "trackId": None,
...
lidar_candidate_id_state: dict[str, Any] = {
    "nextId": 1,
    "keyToId": {},
    "passedIds": [],
    "passedTrackIds": [],
...
recognized_lidar_keys: dict[str, dict[str, Any]] = {}
...
def lidar_turret_yaw_control(lidar_target: dict[str, Any] | None) -> dict[str, Any]:
    """LiDAR-only turret yaw correction using pulse aiming.

    Q = turret left, E = turret right.

    This version intentionally avoids continuous yaw input. The turret sends one
    small command, then rests for TURRET_PULSE_COOLDOWN_SECONDS so the simulator
    and web/API state can catch up. This is the "move once -> wait -> move once"
    behavior requested for reducing left-right oscillation.
    """
    if not lidar_target:
    ...
    long_range_micro_pulse = distance is not None and distance >= TURRET_LONG_RANGE_MICRO_DISTANCE_M

    fire_deadband = distance_based_fire_yaw_deadband(distance)
    stop_deadband = max(
        float(TURRET_LONG_RANGE_STOP_DEADBAND_DEG if long_range_micro_pulse else TURRET_STOP_DEADBAND_DEG),
        fire_deadband,
    )
    ...
    # Stop zone: do not nudge anymore. Hold still and wait until the aim remains
    # stable long enough for fire readiness.
    ...
    # Left stop zone, so settlement must be checked again later.
    aim_stabilizer_state["insideFineSince"] = None
    aim_stabilizer_state["settled"] = False

    # Very small deadband guard from the older controller.
    if abs_error <= LIDAR_TURRET_YAW_DEADBAND_DEG:
        aim_stabilizer_state["reason"] = "inside_lidar_yaw_deadband"
        return {"command": "", "weight": 0.0}

    # Pulse cooldown: one command, then rest. This applies in all zones so the
    # turret cannot continuously swing left/right on stale state.
    pulse_cooldown = (
        TURRET_LONG_RANGE_PULSE_COOLDOWN_SECONDS
        if long_range_micro_pulse
        else TURRET_PULSE_COOLDOWN_SECONDS
    )
    if TURRET_PULSE_AIM_ENABLED:
        ...
        if elapsed < pulse_cooldown:
            aim_stabilizer_state["reason"] = (
                "long_range_micro_pulse_cooldown_waiting"
                if long_range_micro_pulse
                else "pulse_cooldown_waiting_for_fresh_state"
            )
            return {"command": "", "weight": 0.0}

    # Choose one conservative pulse weight by yaw-error zone.
    if abs_error <= fine_zone:
        weight = float(TURRET_LONG_RANGE_WEIGHT_FINE if long_range_micro_pulse else TURRET_PULSE_WEIGHT_FINE)
        reason = "long_range_fine_micro_pulse" if long_range_micro_pulse else "fine_zone_single_micro_pulse"
    elif abs_error <= close_zone:
        weight = float(TURRET_LONG_RANGE_WEIGHT_CLOSE if long_range_micro_pulse else TURRET_PULSE_WEIGHT_CLOSE)
        reason = "long_range_close_micro_pulse" if long_range_micro_pulse else "close_zone_single_pulse"
    else:
        weight = float(TURRET_LONG_RANGE_WEIGHT_FAR if long_range_micro_pulse else TURRET_PULSE_WEIGHT_FAR)
        reason = "long_range_far_micro_pulse" if long_range_micro_pulse else "far_zone_single_pulse"
...
def target_ignore_key(obj: dict[str, Any] | None) -> str | None:
    if not obj:
        return None
    candidate_id = obj.get("lidarCandidateId")
    if candidate_id is not None:
        return f"lidar-candidate:{candidate_id}"
    ...
    pos = obj.get("filteredWorldPosition") or obj.get("worldPosition") or {}
    if isinstance(pos, dict) and pos.get("x") is not None and pos.get("z") is not None:
        x_bucket = round(float(pos["x"]) / 5.0) * 5
        z_bucket = round(float(pos["z"]) / 5.0) * 5
        return f"pos:{x_bucket:.0f}:{z_bucket:.0f}"
    label = obj.get("candidateLabel", "UK")
    distance = safe_float(obj.get("nearestDistance"))
    angle = safe_float(obj.get("centerAngle", obj.get("bodyRelativeAngleErrorDeg")))
    if distance is not None and angle is not None:
        distance_bucket = round(float(distance) / 5.0) * 5
        angle_bucket = round(float(angle) / 5.0) * 5
        return f"candidate:{label}:{distance_bucket:.0f}:{angle_bucket:.0f}"
    return None


def lidar_candidate_stable_key(obj: dict[str, Any]) -> str:
    track_id = obj.get("trackId")
    if track_id is not None:
        return f"track:{track_id}"
    pos = obj.get("filteredWorldPosition") or obj.get("worldPosition") or {}
    if isinstance(pos, dict) and pos.get("x") is not None and pos.get("z") is not None:
        x_bucket = round(float(pos["x"]) / 3.0) * 3
        z_bucket = round(float(pos["z"]) / 3.0) * 3
        return f"pos:{x_bucket:.0f}:{z_bucket:.0f}"
    label = obj.get("candidateLabel", "UK")
    distance = safe_float(obj.get("nearestDistance", obj.get("medianDistance")))
    angle = safe_float(obj.get("centerAngle", obj.get("bodyRelativeAngleErrorDeg")))
    distance_bucket = round(float(distance or 0.0) / 5.0) * 5
    angle_bucket = round(float(angle or 0.0) / 5.0) * 5
    return f"candidate:{label}:{distance_bucket:.0f}:{angle_bucket:.0f}"


def assign_lidar_candidate_id(obj: dict[str, Any]) -> dict[str, Any]:
    key = lidar_candidate_stable_key(obj)
    key_to_id = lidar_candidate_id_state.setdefault("keyToId", {})
    if key not in key_to_id:
        next_id = int(lidar_candidate_id_state.get("nextId", 1) or 1)
        key_to_id[key] = next_id
        lidar_candidate_id_state["nextId"] = next_id + 1
    obj["lidarCandidateId"] = key_to_id[key]
    obj["lidarCandidateKey"] = key
    passed_ids = set(lidar_candidate_id_state.get("passedIds", []))
    passed_track_ids = set(lidar_candidate_id_state.get("passedTrackIds", []))
    track_id = obj.get("trackId")
    obj["passedByYolo"] = (
        not is_recognized_attack_lidar_target(obj)
        and (
            int(obj["lidarCandidateId"]) in passed_ids
            or (track_id is not None and int(track_id) in passed_track_ids)
        )
    )
    return obj


def mark_lidar_candidate_passed(obj: dict[str, Any] | None) -> None:
    if not obj:
        return
    if is_recognized_attack_lidar_target(obj):
        return
    track_id = obj.get("trackId")
    if track_id is not None:
        passed_track_ids = set(lidar_candidate_id_state.get("passedTrackIds", []))
        passed_track_ids.add(int(track_id))
        lidar_candidate_id_state["passedTrackIds"] = sorted(passed_track_ids)
    candidate_id = obj.get("lidarCandidateId")
    if candidate_id is None:
        assigned = assign_lidar_candidate_id(deepcopy(obj))
        candidate_id = assigned.get("lidarCandidateId")
    if candidate_id is None:
        return
    passed = set(lidar_candidate_id_state.get("passedIds", []))
    passed.add(int(candidate_id))
    lidar_candidate_id_state["passedIds"] = sorted(passed)


def clear_lidar_candidate_passes(reason: str = "reset") -> None:
    lidar_candidate_id_state["passedIds"] = []
    lidar_candidate_id_state["passedTrackIds"] = []
    yolo_fire_debounce_state.update({
        "targetKey": None,
        "trackId": None,
        "hits": 0,
        "misses": 0,
        ...
        "className": None,
        "confidence": None,
        "updatedAt": datetime.now().isoformat(timespec="milliseconds"),
        "reason": reason,
    })
...
def mark_target_as_eliminated_after_fire(lidar_target: dict[str, Any] | None) -> None:
    ...
    # Force the next LiDAR summary to select the next priority target.
    body_alignment_state["lockedTrackId"] = None
    body_alignment_state["lockedTargetKey"] = None
...
def fire_readiness_status(now: float | None = None) -> dict[str, Any]:
    ...
    yolo_fire_confirmed = bool(
        yolo_gate_confirmed_for_target(lidar_target)
        and yolo_fire_confirmed_for_track(lidar_target)
    )
    ...
    # Looser UI gate: allow the button to arm a shot near the target, even if
    # strict firing readiness is not true yet. Actual firing still requires
    # ready == True while the approval window is active.
    ...
    elif REQUIRE_YOLO_ENEMY_TANK_FOR_FIRE and not yolo_fire_confirmed:
        reason = yolo_fire_debounce_state.get("reason", "waiting_for_enemy_tank_yolo_debounce")
    ...
    return {
        ...
        "requireYoloEnemyTankForFire": REQUIRE_YOLO_ENEMY_TANK_FOR_FIRE,
        "yoloEnemyTankFireConfirmed": yolo_fire_confirmed,
        "yoloFireDebounce": deepcopy(yolo_fire_debounce_state),
        "yoloRecognitionGate": deepcopy(yolo_recognition_gate_state),
        "passedLidarCandidateIds": deepcopy(lidar_candidate_id_state.get("passedIds", [])),
        "passedLidarTrackIds": deepcopy(lidar_candidate_id_state.get("passedTrackIds", [])),
...
def lidar_target_bind_snapshot(lidar_target: dict[str, Any] | None) -> dict[str, Any]:
    if not lidar_target:
        return {"key": None, "angle": None}
    angle = safe_float(
        lidar_target.get("bodyRelativeAngleErrorDeg", lidar_target.get("centerAngle"))
    )
    return {
...
def lidar_target_stable_for_vision_result(
    before: dict[str, Any],
    after: dict[str, Any],
) -> bool:
    ...
    if before.get("key") != after.get("key"):
        return False
    before_angle = safe_float(before.get("angle"))
    after_angle = safe_float(after.get("angle"))
    if before_angle is None or after_angle is None:
        return True
    drift = abs(normalize_signed_angle(float(after_angle) - float(before_angle)))
    return drift <= YOLO_LIDAR_BIND_MAX_TARGET_ANGLE_DRIFT_DEG


def yolo_gate_target_matches(lidar_target: dict[str, Any] | None) -> bool:
    if not lidar_target:
        return False
    target_key = target_ignore_key(lidar_target)
    track_id = lidar_target.get("trackId")
    return bool(
        (target_key is not None and yolo_recognition_gate_state.get("targetKey") == target_key)
        or (track_id is not None and yolo_recognition_gate_state.get("trackId") == track_id)
    )


def yolo_gate_confirmed_for_target(lidar_target: dict[str, Any] | None) -> bool:
    return bool(
        yolo_gate_target_matches(lidar_target)
        and yolo_recognition_gate_state.get("phase") == "confirmed"
    )


def yolo_fire_debounce_matches_target(lidar_target: dict[str, Any] | None) -> bool:
    if not lidar_target:
        return False
    target_key = target_ignore_key(lidar_target)
    track_id = lidar_target.get("trackId")
    return bool(
        yolo_fire_debounce_state.get("confirmed")
        and (
            (target_key is not None and yolo_fire_debounce_state.get("targetKey") == target_key)
            or (track_id is not None and yolo_fire_debounce_state.get("trackId") == track_id)
        )
    )


def yolo_gate_holds_current_target() -> bool:
    return str(yolo_recognition_gate_state.get("phase", "")) in {
        "post_align_delay",
        "judging",
        "confirmed",
    }
...
def yolo_gate_can_confirm_locked_target(
    lidar_target: dict[str, Any] | None,
    fusion: dict[str, Any] | None,
) -> bool:
    if not lidar_target or not fusion:
        return False
    return bool(
        yolo_gate_target_matches(lidar_target)
        and str(yolo_recognition_gate_state.get("phase", "")) == "judging"
        and fusion.get("alignedForFusion", False)
        and fusion.get("lidarGeometryMatchesYolo", False)
        and is_attack_yolo_class((fusion.get("vision") or {}).get("className"))
    )


def update_yolo_recognition_gate(
    lidar_target: dict[str, Any] | None,
    aligned: bool,
    now: float | None = None,
) -> dict[str, Any]:
    now = monotonic() if now is None else now
    target_key = target_ignore_key(lidar_target)
    track_id = (lidar_target or {}).get("trackId")

    if target_key is None and track_id is None:
        yolo_recognition_gate_state.update({
            "targetKey": None,
            "trackId": None,
            "phase": "waiting_for_target",
            "alignedSince": None,
        ...
        return deepcopy(yolo_recognition_gate_state)
    ...
    if not same_target:
        yolo_recognition_gate_state.update({
            ...
            "allowYolo": False,
            "decisionApplied": False,
            "reason": "new_target_align_before_yolo",
        })
        vision_state["target"] = None
        vision_state["lidarFusion"] = None

    if yolo_recognition_gate_state.get("phase") in ("confirmed", "rejected") and same_target:
        yolo_recognition_gate_state["allowYolo"] = False
        return deepcopy(yolo_recognition_gate_state)

    if yolo_fire_debounce_matches_target(lidar_target):
        yolo_recognition_gate_state.update({
            "phase": "confirmed",
            "allowYolo": False,
            "decisionApplied": False,
            "reason": "tank_confirmed_by_yolo_detection",
        })
        return deepcopy(yolo_recognition_gate_state)

    angle_error = safe_float(
        (lidar_target or {}).get("bodyRelativeAngleErrorDeg", (lidar_target or {}).get("centerAngle"))
    )
    prealigned_for_yolo = bool(
        angle_error is not None
        and abs(float(angle_error)) <= YOLO_RECOGNITION_PREALIGN_DEG
    )
    if prealigned_for_yolo:
        aligned = True
    ...
    if not aligned:
        yolo_recognition_gate_state.update({
            ...
            "judgeDeadline": None,
            "allowYolo": False,
            "reason": "aligning_target_before_yolo",
        })
        return deepcopy(yolo_recognition_gate_state)

    aligned_since = yolo_recognition_gate_state.get("alignedSince")
    if aligned_since is None:
        aligned_since = now
        yolo_recognition_gate_state["alignedSince"] = aligned_since

    delay_remaining = float(aligned_since) + YOLO_RECOGNITION_DELAY_AFTER_ALIGN_SECONDS - now
    if delay_remaining > 0.0:
        yolo_recognition_gate_state.update({
            "phase": "post_align_delay",
            "judgeStartedAt": None,
            "judgeDeadline": None,
            "allowYolo": False,
            "reason": "waiting_after_alignment_before_yolo",
            "delayRemainingSeconds": round(delay_remaining, 3),
        })
        return deepcopy(yolo_recognition_gate_state)

    judge_started_at = yolo_recognition_gate_state.get("judgeStartedAt")
    if judge_started_at is None:
        judge_started_at = now
        yolo_recognition_gate_state["judgeStartedAt"] = judge_started_at
        yolo_recognition_gate_state["judgeDeadline"] = now + YOLO_RECOGNITION_JUDGE_SECONDS

    judge_deadline = float(yolo_recognition_gate_state.get("judgeDeadline") or (float(judge_started_at) + YOLO_RECOGNITION_JUDGE_SECONDS))
    if now <= judge_deadline:
        yolo_recognition_gate_state.update({
            "phase": "judging",
            "allowYolo": True,
            "reason": "yolo_judgement_window_open",
            "judgeRemainingSeconds": round(judge_deadline - now, 3),
        })
        return deepcopy(yolo_recognition_gate_state)
    ...
    yolo_recognition_gate_state.update({
        "phase": "confirmed" if confirmed else "rejected",
        "allowYolo": False,
        "reason": "tank_confirmed_after_2s_yolo_judgement" if confirmed else "not_tank_after_2s_yolo_judgement",
    })
    return deepcopy(yolo_recognition_gate_state)
...
def build_lidar_vision_fusion(
    ...
    geometry_matches = is_attack_target or mapped_geometry is None or mapped_geometry == lidar_geometry
    on_current_aim = yolo_detection_on_current_aim(vision_target)
    locked_gate_match = yolo_gate_target_matches(lidar_target)
    relaxed_current_target_confirmation = bool(
        is_attack_target
        and geometry_matches
        and aligned_for_fusion
        and locked_gate_match
    )
    recognition_alignment_candidate = bool(
        not aligned_for_fusion
        and lidar_target.get("recognitionAlignmentOnly")
        and is_attack_target
        and geometry_matches
    )
    recognized_attack_target = bool(
        is_attack_target
        and geometry_matches
        and (on_current_aim or relaxed_current_target_confirmation)
    )
    fusion.update({
        "status": (
            "recognized_attack_target"
            if recognized_attack_target
            else (
                "recognized_attack_target_alignment_pending"
                if recognition_alignment_candidate
                else (
                    "recognized_geometry_mismatch"
                    if not geometry_matches
                    else (
                        "recognized_attack_target_off_current_aim"
                        if is_attack_target and not on_current_aim
        ...
        "yoloOnCurrentAim": on_current_aim,
        "relaxedCurrentTargetConfirmation": relaxed_current_target_confirmation,
        "recognitionAlignmentCandidate": recognition_alignment_candidate,
        "recognizedAttackTarget": recognized_attack_target,
        "yoloAimTolerance": {"x": YOLO_FIRE_CENTER_X_TOLERANCE, "y": YOLO_FIRE_CENTER_Y_TOLERANCE},
        "isAttackTarget": recognized_attack_target,
    })
    return fusion
...
def is_attack_yolo_class(class_name: Any) -> bool:
    semantic = normalize_yolo_class_name(class_name)
    return semantic in VISION_TARGET_CLASSES


def yolo_detection_on_current_aim(vision_target: dict[str, Any] | None) -> bool:
    if not vision_target:
        return False
    error_x = abs(safe_float(vision_target.get("errorX"), 999.0))
    error_y = abs(safe_float(vision_target.get("errorY"), 999.0))
    return error_x <= YOLO_FIRE_CENTER_X_TOLERANCE and error_y <= YOLO_FIRE_CENTER_Y_TOLERANCE


def yolo_fire_confirmed_for_track(lidar_target: dict[str, Any] | None) -> bool:
    if not REQUIRE_YOLO_ENEMY_TANK_FOR_FIRE:
        return True
    if not lidar_target:
        return False
    target_key = target_ignore_key(lidar_target)
    ...
    if target_key is not None:
        recognition = recognized_lidar_keys.get(target_key)
        if recognition and is_attack_yolo_class(recognition.get("className")):
            yolo_fire_debounce_state.update({
                "targetKey": target_key,
                "trackId": track_id,
                "hits": max(int(yolo_fire_debounce_state.get("hits", 0) or 0), YOLO_FIRE_DEBOUNCE_HITS),
                "misses": 0,
                "confirmed": True,
                "rejected": False,
                "className": recognition.get("className"),
                "confidence": recognition.get("confidence"),
                "updatedAt": datetime.now().isoformat(timespec="milliseconds"),
                "reason": "enemy_tank_key_cache_confirmed",
            })
            return True
    if track_id is not None:
        ...
        if recognition and is_attack_yolo_class(recognition.get("className")):
            yolo_fire_debounce_state.update({
                ...
                "confidence": recognition.get("confidence"),
                "updatedAt": datetime.now().isoformat(timespec="milliseconds"),
                "reason": "enemy_tank_recognition_cache_confirmed",
            })
            return True
    return bool(
        yolo_fire_debounce_state.get("confirmed")
...
def mark_target_as_rejected_by_yolo(lidar_target: dict[str, Any] | None) -> None:
    if (
        yolo_fire_debounce_matches_target(lidar_target)
        or yolo_gate_confirmed_for_target(lidar_target)
        or is_recognized_attack_lidar_target(lidar_target)
    ):
        yolo_recognition_gate_state.update({
            "phase": "confirmed",
            "allowYolo": False,
            "decisionApplied": False,
            "reason": "reject_suppressed_for_confirmed_tank",
        })
        return
    key = target_ignore_key(lidar_target)
    mark_lidar_candidate_passed(lidar_target)
    if key is None:
        return
    now = monotonic()
    cleanup_eliminated_targets(now)
    ignored = eliminated_target_state.get("ignoredTracks", {}) or {}
    ignored[key] = now + YOLO_REJECT_IGNORE_SECONDS
    eliminated_target_state.update({
        "ignoredTracks": ignored,
        "lastRejectedTrackId": (lidar_target or {}).get("trackId"),
        "lastRejectedAt": datetime.now().isoformat(timespec="milliseconds"),
    })
    body_alignment_state["lockedTrackId"] = None
    body_alignment_state["lockedTargetKey"] = None
    body_alignment_state["target"] = None
    body_alignment_state["moveAD"] = {"command": "", "weight": 0.0}
    clear_world_target_lock("yolo_rejected_non_enemy_tank_switch_target")
    reset_aim_stabilizer("yolo_rejected_non_enemy_tank_switch_target")


def update_yolo_fire_debounce(
    lidar_target: dict[str, Any] | None,
    fusion: dict[str, Any] | None,
) -> None:
    if not REQUIRE_YOLO_ENEMY_TANK_FOR_FIRE or not lidar_target:
        return
    target_key = target_ignore_key(lidar_target)
    track_id = lidar_target.get("trackId")
    if target_key is None and track_id is None:
        return
    same_debounce_target = bool(
        (target_key is not None and yolo_fire_debounce_state.get("targetKey") == target_key)
        or (track_id is not None and yolo_fire_debounce_state.get("trackId") == track_id)
    )
    if not same_debounce_target:
        yolo_fire_debounce_state.update({
            "targetKey": target_key,
            "trackId": track_id,
            "hits": 0,
            ...
            "confidence": None,
            "updatedAt": None,
            "reason": "new_track_waiting_for_detection",
        })

    vision = (fusion or {}).get("vision") or {}
    class_name = str(vision.get("className", "unknown"))
    confidence = safe_float(vision.get("confidence"), 0.0) or 0.0
    has_detection = bool(vision)
    off_current_enemy_tank = bool(
        has_detection
        and is_attack_yolo_class(class_name)
        and not (fusion or {}).get("yoloOnCurrentAim")
        and not (fusion or {}).get("recognitionAlignmentCandidate")
        and confidence >= VISION_CONFIDENCE_MIN
    )
    enemy_tank = bool(
        (
            (fusion or {}).get("isAttackTarget")
            or (fusion or {}).get("recognizedAttackTarget")
            or yolo_gate_can_confirm_locked_target(lidar_target, fusion)
        )
        and is_attack_yolo_class(class_name)
        and ((fusion or {}).get("yoloOnCurrentAim") or yolo_gate_can_confirm_locked_target(lidar_target, fusion))
        and (fusion or {}).get("alignedForFusion")
        and confidence >= VISION_CONFIDENCE_MIN
    )
    ready_to_judge_current_target = bool(
        (fusion or {}).get("alignedForFusion")
        or (lidar_target.get("recognitionAlignmentOnly") and has_detection)
    )

    if enemy_tank:
        hits = int(yolo_fire_debounce_state.get("hits", 0) or 0) + 1
        confirmed = hits >= YOLO_FIRE_DEBOUNCE_HITS
        yolo_fire_debounce_state.update({
            "hits": hits,
            "misses": 0,
            "confirmed": confirmed,
            ...
            "updatedAt": datetime.now().isoformat(timespec="milliseconds"),
            "reason": "enemy_tank_debounce_confirmed" if confirmed else "enemy_tank_debounce_pending",
        })
        if confirmed:
            yolo_recognition_gate_state.update({
                "targetKey": target_key,
    ...
    if off_current_enemy_tank:
        yolo_fire_debounce_state.update({
            ...
            "confidence": round(confidence, 4),
            "updatedAt": datetime.now().isoformat(timespec="milliseconds"),
            "reason": "enemy_tank_seen_off_current_aim_keep_aligning",
        })
        return

    if ready_to_judge_current_target:
        misses = int(yolo_fire_debounce_state.get("misses", 0) or 0) + 1
        rejected = misses >= YOLO_FIRE_DEBOUNCE_MISSES
        no_detection = not has_detection
        yolo_fire_debounce_state.update({
            "hits": 0,
            "misses": misses,
            "confirmed": False,
            "rejected": rejected,
            "className": None if no_detection else class_name,
            "confidence": None if no_detection else round(confidence, 4),
            "updatedAt": datetime.now().isoformat(timespec="milliseconds"),
            "reason": (
                "no_enemy_tank_detection_rejected"
                if rejected and no_detection
                else (
                    "non_enemy_tank_rejected"
                    if rejected
                    else (
                        "no_enemy_tank_detection_debounce_pending"
                        if no_detection
                        else "non_enemy_tank_debounce_pending"
                    )
                )
            ),
        })
        if rejected and yolo_recognition_gate_state.get("phase") != "judging":
            mark_target_as_rejected_by_yolo(lidar_target)
...
def enrich_with_recognition(obj: dict[str, Any]) -> dict[str, Any]:
    ...
    if track_id is not None:
        recognition = recognized_lidar_objects.get(int(track_id))
    if not recognition:
        target_key = target_ignore_key(obj)
        if target_key is not None:
            recognition = recognized_lidar_keys.get(target_key)
    if not recognition:
        return obj
...
def recognition_for_lidar_target(obj: dict[str, Any] | None) -> dict[str, Any] | None:
    if not obj:
        return None
    track_id = obj.get("trackId")
    if track_id is not None:
        ...
        if recognition:
            return recognition
    for key in (target_ignore_key(obj), obj.get("lidarCandidateKey"), lidar_candidate_stable_key(obj)):
        if key is None:
            continue
        recognition = recognized_lidar_keys.get(str(key))
        if recognition:
            return recognition
    return None


def is_recognized_attack_lidar_target(obj: dict[str, Any] | None) -> bool:
    if not obj:
        return False
    if is_attack_yolo_class(obj.get("semanticClass")) or is_attack_yolo_class(obj.get("recognizedClass")):
        return True
    recognition = recognition_for_lidar_target(obj)
    return bool(recognition and is_attack_yolo_class(recognition.get("className")))
...
def remember_recognized_lidar_object(
    ...
    if not fusion.get("lidarGeometryMatchesYolo", False) and not fusion.get("isAttackTarget", False):
        return
    if (
        not fusion.get("yoloOnCurrentAim", False)
        and not fusion.get("relaxedCurrentTargetConfirmation", False)
        and not yolo_gate_can_confirm_locked_target(lidar_target, fusion)
    ):
        return
    if not fusion.get("alignedForFusion", False):
        return
    target_key = target_ignore_key(lidar_target)
    track_id = lidar_target.get("trackId")
    ...
    recognition = {
        ...
        "isAttackTarget": fusion.get("isAttackTarget", False) or fusion.get("recognizedAttackTarget", False),
        "recognizedAt": datetime.now().isoformat(timespec="milliseconds"),
    }
    if target_key is not None:
        recognized_lidar_keys[target_key] = deepcopy(recognition)
    stable_key = lidar_candidate_stable_key(lidar_target)
    if stable_key is not None:
        recognized_lidar_keys[stable_key] = deepcopy(recognition)
    lidar_candidate_key = lidar_target.get("lidarCandidateKey")
    if lidar_candidate_key is not None:
        recognized_lidar_keys[str(lidar_candidate_key)] = deepcopy(recognition)
    if track_id is not None:
        recognized_lidar_objects[int(track_id)] = deepcopy(recognition)
...
def classify_candidate(
    ...
    if width <= THIN_MAX_WIDTH_M:
        if (
            distance >= FAR_SPARSE_TANK_MIN_DISTANCE_M
            and point_count >= FAR_SPARSE_TANK_MIN_POINTS
            and height_span >= FAR_SPARSE_TANK_MIN_HEIGHT_SPAN_M
        ):
            return "bulky", "medium", "far_sparse_tank_possible"
...
def summarize_object_cluster(cluster: list[dict[str, Any]], object_id: int, sectors: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    geometry, confidence, reason = classify_candidate(width, median_distance, len(points), height)
...
def reset_runtime_state() -> None:
    ...
    global object_tracker, latest_state, latest_raw_info, latest_info_received_at, body_alignment_state, action_debug_state, vision_state, yolo_fire_debounce_state, yolo_recognition_gate_state, lidar_candidate_id_state, recognized_lidar_objects, recognized_lidar_keys, fire_control_state, aim_zero_state, aim_stabilizer_state, world_target_lock_state, eliminated_target_state, next_impact_id

    terrain_history.clear()
    impact_history.clear()
    detect_timing_history.clear()
    next_impact_id = 1
    object_tracker = ObjectTracker()
    recognized_lidar_objects = {}
    recognized_lidar_keys = {}
    latest_raw_info = {}
    latest_info_received_at = 0.0
    latest_state = {
        "simulationTime": None,
...
def enrich_priority_candidate(
    ...
    enriched = enrich_with_recognition(assign_lidar_candidate_id(deepcopy(obj)))
    ...
    angle_error = abs(normalize_signed_angle(float(enriched.get("centerAngle", 0.0))))
    semantic_rank = 0 if semantic_is_tank_candidate(enriched) else 1
    front_rank = 0 if angle_error <= PRIORITY_FRONT_PREFERRED_ANGLE_DEG else 1

    # Prefer recognized tanks, then bulky/tank-like LiDAR shapes, then distance.
    # Front angle is still useful, but it must not make a thin/unknown object
    # steal priority from a likely tank candidate.
    geometry_rank = priority_geometry_rank(geometry)
    near_rank = 0 if is_close else 1
    priority_key = (
        semantic_rank,
        geometry_rank,
        near_rank,
...
def make_fusion_priority_queue(
    ...
    for obj in confirmed_objects:
        ...
        if candidate.get("passedByYolo") and not semantic_is_tank_candidate(candidate):
            continue
    ...
    for obj in provisional_objects:
        ...
        if provisional.get("passedByYolo") and not semantic_is_tank_candidate(provisional):
            continue
...
def semantic_is_tank_candidate(obj: dict[str, Any]) -> bool:
    ...
    Current lalast.pt attack semantics:
    - Enemy
    - Tank_enemy
    """
...
def is_body_align_candidate(obj: dict[str, Any]) -> bool:
    ...
    if REQUIRE_YOLO_ENEMY_TANK_FOR_FIRE and obj.get("passedByYolo"):
        return False
...
def body_align_target_sort_key(obj: dict[str, Any]) -> tuple[int, int, int, float, int, float, int]:
    """Prefer recognized attack targets and bulky shapes before front angle."""
    distance = float(obj.get("nearestDistance", MAX_DISTANCE_M))
    angle_error = abs(normalize_signed_angle(float(obj.get("centerAngle", 0.0))))
    front_rank = 0 if angle_error <= PRIORITY_FRONT_PREFERRED_ANGLE_DEG else 1
    confirmed_rank = 0 if obj.get("trackingStage") == "confirmed" else 1
    semantic_rank = 0 if semantic_is_tank_candidate(obj) else 1
    geometry_rank = priority_geometry_rank(str(obj.get("geometryClass", "unknown")))
    near_rank = 0 if distance <= PRIORITY_NEAR_MAX_DISTANCE_M else 1
    return semantic_rank, geometry_rank, near_rank, distance, front_rank, angle_error, confirmed_rank
...
def body_align_spatial_match(
    ...
    angle = safe_float(obj.get("centerAngle", obj.get("bodyRelativeAngleErrorDeg")))
    distance = safe_float(obj.get("nearestDistance", obj.get("medianDistance")))
    locked_angle = safe_float(locked_angle)
    locked_distance = safe_float(locked_distance)
    if angle is None or distance is None or locked_angle is None or locked_distance is None:
        return False
    angle_delta = abs(normalize_signed_angle(float(angle) - float(locked_angle)))
    distance_delta = abs(float(distance) - float(locked_distance))
    return (
...
def choose_body_align_target(
    ...
    if not candidates:
        ...
        if BODY_ALIGN_USE_PRIMARY_FOR_RECOGNITION and priority_queue:
            ...
            body_alignment_state["lockedTargetKey"] = target_ignore_key(selected)
            body_alignment_state["lockedCenterAngle"] = selected.get("centerAngle")
            body_alignment_state["lockedDistance"] = selected.get("nearestDistance")
            selected["recognitionAlignmentOnly"] = True
            return selected
        return None

    locked_track_id = body_alignment_state.get("lockedTrackId")
    locked_target_key = body_alignment_state.get("lockedTargetKey")
    locked_angle = body_alignment_state.get("lockedCenterAngle")
    locked_distance = body_alignment_state.get("lockedDistance")
    if locked_track_id is not None:
        for obj in candidates:
            if obj.get("trackId") == locked_track_id:
                ...
                return deepcopy(obj)
    if locked_target_key is not None:
        for obj in candidates:
            if target_ignore_key(obj) == locked_target_key:
                body_alignment_state["lockedCenterAngle"] = obj.get("centerAngle")
                body_alignment_state["lockedDistance"] = obj.get("nearestDistance")
                return deepcopy(obj)
        for obj in candidates:
            if body_align_spatial_match(obj, locked_angle, locked_distance):
                body_alignment_state["lockedTrackId"] = obj.get("trackId")
                body_alignment_state["lockedTargetKey"] = target_ignore_key(obj)
                body_alignment_state["lockedCenterAngle"] = obj.get("centerAngle")
                body_alignment_state["lockedDistance"] = obj.get("nearestDistance")
                return deepcopy(obj)
...
def summarize_lidar(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if not fusion_priority_queue and (confirmed_objects or provisional_objects):
        clear_lidar_candidate_passes("priority_queue_empty_rebuild_candidates")
        eliminated_target_state["ignoredTracks"] = {}
        fusion_priority_queue, primary_fusion_target = make_fusion_priority_queue(
            confirmed_objects=confirmed_objects,
            provisional_objects=provisional_objects,
        )
...
def detect_visual_targets(image: Image.Image) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    ...
    for result in results:
        ...
        for box in result.boxes:
            ...
            if not is_attack_yolo_class(class_name):
                continue
            ...
            aim_center_distance = hypot(error_x, error_y)
            in_decision_center = (
            ...
            detections.append({
                ...
                "aimCenterDistance": round(aim_center_distance, 4),
                "area": round(area, 1),
    ...
    target_candidates.sort(key=lambda item: (
        abs(safe_float(item.get("errorX"), 999.0)),
        abs(safe_float(item.get("errorY"), 999.0)),
        -safe_float(item.get("confidence"), 0.0),
        -safe_float(item.get("area"), 0.0),
    ))
    return detections, deepcopy(target_candidates[0])


def annotate_detections_with_lidar_distance(
    detections: list[dict[str, Any]],
    lidar_target: dict[str, Any] | None,
) -> list[dict[str, Any]]:
    distance = safe_float((lidar_target or {}).get("nearestDistance"))
    ...
    for detection in detections:
        if not is_attack_yolo_class(detection.get("className")):
            continue
        detection["lidarDistance"] = round(float(distance), 1)
        detection["distanceText"] = distance_text
        detection["displayName"] = f"{detection['className']} {distance_text}"
    return detections


def timing_avg(values: list[float]) -> float | None:
    return round(sum(values) / len(values), 3) if values else None


def timing_max(values: list[float]) -> float | None:
    return round(max(values), 3) if values else None


def timing_p95(values: list[float]) -> float | None:
    if not values:
        return None
...
def numeric_timing_values(rows: list[dict[str, Any]], key: str) -> list[float]:
    values = []
    for row in rows:
        value = row.get(key)
...
def detect_timing_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    if not rows:
        return {
            "count": 0,
            "message": "No /detect timing samples yet.",
        }

    yolo_rows = [row for row in rows if row.get("allowYolo")]
    state_to_finish = numeric_timing_values(rows, "stateToDetectFinishMs")
    yolo_latency = numeric_timing_values(yolo_rows, "yoloLatencyMs")
    handler_latency = numeric_timing_values(rows, "handlerLatencyMs")
    info_age = numeric_timing_values(rows, "latestInfoAgeAtDetectStartMs")

    return {
        "count": len(rows),
        "yoloCount": len(yolo_rows),
        "gateClosedCount": len(rows) - len(yolo_rows),
        "latest": deepcopy(rows[-1]),
        "latestSimulationTime": rows[-1].get("latestSimulationTime"),
        "latestInfoAgeAtDetectStartMs": {
            "avg": timing_avg(info_age),
            "p95": timing_p95(info_age),
            "max": timing_max(info_age),
        },
        "yoloLatencyMs": {
            "avg": timing_avg(yolo_latency),
            "p95": timing_p95(yolo_latency),
            "max": timing_max(yolo_latency),
        },
        "stateToDetectFinishMs": {
            "avg": timing_avg(state_to_finish),
            "p95": timing_p95(state_to_finish),
            "max": timing_max(state_to_finish),
        },
        "handlerLatencyMs": {
            "avg": timing_avg(handler_latency),
            "p95": timing_p95(handler_latency),
            "max": timing_max(handler_latency),
        },
        "acceptedForLidarCount": sum(1 for row in rows if row.get("visionResultAcceptedForLidar")),
    }


def record_detect_timing(
    *,
    request_received_at: float,
    image_decoded_at: float,
    yolo_started_at: float | None,
    yolo_finished_at: float | None,
    response_ready_at: float,
    allow_yolo: bool,
    detection_count: int,
    gate: dict[str, Any] | None,
    result_fresh_for_lidar: bool | None = None,
    vision_result_accepted_for_lidar: bool | None = None,
) -> dict[str, Any]:
    info_age = request_received_at - latest_info_received_at if latest_info_received_at else None
    state_to_finish = response_ready_at - latest_info_received_at if latest_info_received_at else None
    yolo_latency = yolo_finished_at - yolo_started_at if yolo_started_at is not None and yolo_finished_at is not None else None
    row = {
        "recordedAt": datetime.now().isoformat(timespec="milliseconds"),
        "latestSimulationTime": latest_state.get("simulationTime"),
        "latestInfoAgeAtDetectStartMs": round(info_age * 1000.0, 3) if info_age is not None else None,
        "stateToDetectFinishMs": round(state_to_finish * 1000.0, 3) if state_to_finish is not None else None,
        "imageDecodeMs": round((image_decoded_at - request_received_at) * 1000.0, 3),
        "yoloLatencyMs": round(yolo_latency * 1000.0, 3) if yolo_latency is not None else None,
        "handlerLatencyMs": round((response_ready_at - request_received_at) * 1000.0, 3),
        "allowYolo": allow_yolo,
        "gatePhase": (gate or {}).get("phase"),
        "gateReason": (gate or {}).get("reason"),
        "detectionCount": detection_count,
        "resultFreshForLidar": result_fresh_for_lidar,
        "visionResultAcceptedForLidar": vision_result_accepted_for_lidar,
    }
    detect_timing_history.append(row)
    vision_state["detectTiming"] = deepcopy(row)
    latest_state["detectTiming"] = deepcopy(row)
    return row
...
def info():
    info_received_at = monotonic()
    data = request.get_json(force=True, silent=True)
    if not data:
        return jsonify({"error": "No JSON received"}), 400
    scan = summarize_lidar(data)
    with state_lock:
        global latest_state, latest_raw_info, latest_info_received_at
        latest_state = scan
        latest_raw_info = deepcopy(data)
        latest_info_received_at = info_received_at
    print_status(scan)
...
def detect():
    detect_request_received_at = monotonic()
    image = image_from_request()
    image_decoded_at = monotonic()
    if image is None:
        return jsonify({"status": "error", "message": "No image received"}), 400

    detect_started_at = monotonic()
    with state_lock:
        lidar_target_before_detection = current_lidar_fusion_target(latest_state)
        lidar_bind_before_detection = lidar_target_bind_snapshot(lidar_target_before_detection)
        alignment = latest_state.get("bodyAlignment", {}) or {}
        aligned_for_gate = bool(
        ...
        gate = update_yolo_recognition_gate(
            lidar_target_before_detection,
            aligned_for_gate,
            detect_started_at,
        )
        if gate.get("phase") == "rejected" and not gate.get("decisionApplied"):
            mark_target_as_rejected_by_yolo(lidar_target_before_detection)
            yolo_recognition_gate_state["decisionApplied"] = True
            gate = deepcopy(yolo_recognition_gate_state)

        if not gate.get("allowYolo", False):
            fusion = build_lidar_vision_fusion(lidar_target_before_detection, None)
            if fusion is not None:
                fusion.update({
                    "status": gate.get("reason", gate.get("phase", "yolo_gate_closed")),
                    "yoloRecognitionGate": deepcopy(gate),
                    "visionResultAcceptedForLidar": False,
                })
            vision_state["detections"] = []
            vision_state["target"] = None
            vision_state["lidarFusion"] = fusion
            vision_state["modelPath"] = str(MODEL_PATH)
            vision_state["modelLoaded"] = yolo_model is not None
            ...
            latest_state["yoloRecognitionGate"] = deepcopy(gate)
            timing = record_detect_timing(
                request_received_at=detect_request_received_at,
                image_decoded_at=image_decoded_at,
                yolo_started_at=None,
                yolo_finished_at=None,
                response_ready_at=monotonic(),
                allow_yolo=False,
                detection_count=0,
                gate=gate,
                result_fresh_for_lidar=None,
                vision_result_accepted_for_lidar=False,
            )

            debug_payload = {
                "status": "success",
                "model": MODEL_PATH.name,
                "confidenceThreshold": VISION_CONFIDENCE_MIN,
                "detections": [],
                "target": None,
            ...
            return jsonify(debug_payload) if request.args.get("format") == "debug" else jsonify([])

    yolo_started_at = monotonic()
    detections, target = detect_visual_targets(image)
    yolo_finished_at = monotonic()
    now = yolo_finished_at
    with state_lock:
        lidar_target = current_lidar_fusion_target(latest_state)
        lidar_bind_after_detection = lidar_target_bind_snapshot(lidar_target)
        result_fresh_for_lidar = (now - detect_started_at) <= YOLO_LIDAR_BIND_MAX_RESULT_AGE_SECONDS
        target_stable_for_vision = lidar_target_stable_for_vision_result(
            lidar_bind_before_detection,
            lidar_bind_after_detection,
        )
        accept_vision_for_lidar = result_fresh_for_lidar and target_stable_for_vision

        detections = annotate_detections_with_lidar_distance(detections, lidar_target)
        if target:
            target = annotate_detections_with_lidar_distance([target], lidar_target)[0]
        ...
        gate = update_yolo_recognition_gate(lidar_target, aligned_for_gate, now)
        if fusion is not None:
            fusion.update({
                "visionResultAgeSeconds": round(now - detect_started_at, 4),
                "visionResultFreshForLidar": result_fresh_for_lidar,
                "lidarTargetStableForVision": target_stable_for_vision,
                "visionResultAcceptedForLidar": accept_vision_for_lidar,
                "lidarBindBeforeVision": deepcopy(lidar_bind_before_detection),
                "lidarBindAfterVision": deepcopy(lidar_bind_after_detection),
                "yoloRecognitionGate": deepcopy(gate),
            })
            if not accept_vision_for_lidar:
                fusion.update({
                    "status": "stale_vision_result_ignored_for_lidar",
                    "isAttackTarget": False,
                    "recognizedAttackTarget": False,
                    "recognitionAlignmentCandidate": False,
                })
        fused_target = target if fusion and fusion.get("isAttackTarget") else None
        if accept_vision_for_lidar and gate.get("allowYolo", False):
            remember_recognized_lidar_object(lidar_target, target, fusion)
            update_yolo_fire_debounce(lidar_target, fusion)
            gate = update_yolo_recognition_gate(lidar_target, aligned_for_gate, monotonic())
        elif lidar_target:
            yolo_fire_debounce_state.update({
                "targetKey": target_ignore_key(lidar_target),
                "trackId": lidar_target.get("trackId"),
                "hits": 0,
                "misses": 0,
                "confirmed": False,
                ...
                "confidence": None,
                "updatedAt": datetime.now().isoformat(timespec="milliseconds"),
                "reason": "stale_vision_result_ignored_for_lidar",
            })
        if gate.get("phase") == "rejected" and not gate.get("decisionApplied"):
            mark_target_as_rejected_by_yolo(lidar_target)
            yolo_recognition_gate_state["decisionApplied"] = True
        ...
        vision_state["target"] = fused_target if USE_YOLO_FOR_AIM else None
        vision_state["lidarFusion"] = fusion
        vision_state["lastDetectedAt"] = now if fused_target and USE_YOLO_FOR_AIM else vision_state.get("lastDetectedAt", 0.0)
        vision_state["modelPath"] = str(MODEL_PATH)
        vision_state["modelLoaded"] = True
        refresh_latest_state_recognitions()
        latest_state["visionDetections"] = deepcopy(detections)
        latest_state["visionTarget"] = deepcopy(fused_target if USE_YOLO_FOR_AIM else None)
        latest_state["lidarVisionFusion"] = deepcopy(fusion)
        latest_state["yoloRecognitionGate"] = deepcopy(gate)
        timing = record_detect_timing(
            request_received_at=detect_request_received_at,
            image_decoded_at=image_decoded_at,
            yolo_started_at=yolo_started_at,
            yolo_finished_at=yolo_finished_at,
            response_ready_at=monotonic(),
            allow_yolo=True,
            detection_count=len(detections),
            gate=gate,
            result_fresh_for_lidar=result_fresh_for_lidar,
            vision_result_accepted_for_lidar=accept_vision_for_lidar,
        )
    ...
    return jsonify({
        ...
        {
            "className": item.get("displayName", item["className"]),
            "bbox": item["bbox"],
            "confidence": item["confidence"],
            "distance": item.get("lidarDistance"),
            "distanceText": item.get("distanceText"),
...
@app.route("/detect_timing_status", methods=["GET"])
def detect_timing_status():
    try:
        limit = max(1, min(200, int(request.args.get("limit", 50))))
    except (TypeError, ValueError):
        limit = 50
    with state_lock:
        rows = list(detect_timing_history)[-limit:]
        summary = detect_timing_summary(rows)
...
def priority_status():
    ...
    with state_lock:
        queue = deepcopy(latest_state.get("fusionPriorityQueue", []))
        primary = deepcopy(latest_state.get("primaryFusionTarget"))
        if not queue and (latest_state.get("confirmedObjects") or latest_state.get("provisionalObjects")):
            queue, primary = make_fusion_priority_queue(
                confirmed_objects=latest_state.get("confirmedObjects", []),
                provisional_objects=latest_state.get("provisionalObjects", []),
            )
            latest_state["fusionPriorityQueue"] = deepcopy(queue)
            latest_state["fusionPriorityQueueCount"] = len(queue)
            latest_state["primaryFusionTarget"] = deepcopy(primary)
        return jsonify(
            {
                "nearPriorityMaxDistanceM": PRIORITY_NEAR_MAX_DISTANCE_M,
                "primaryFusionTarget": primary,
                "fusionPriorityQueue": queue,
            }
        )
...
function draw(scan) {
  ...
  const priorityLines=(scan.fusionPriorityQueue||[]).slice(0,12).map(o =>
    '#'+o.fusionPriorityRank
      +' | LIDAR-ID '+(o.lidarCandidateId == null ? 'n/a' : o.lidarCandidateId)
      +' | '+objectLabel(o)
...
def get_action():
    ...
    Turret yaw uses a delay-compensated stabilizer near the target.
    Automatic firing is disabled. The FIRE button arms a shot; firing occurs
    while that manual approval is active and the target is close enough to aim.
    """
    ...
    with state_lock:
        ...
        yolo_gate = deepcopy(yolo_recognition_gate_state)

    turret_qe = {"command": "", "weight": 0.0}
    turret_rf = {"command": "", "weight": 0.0}
    gate_phase = str(yolo_gate.get("phase", ""))

    if vision_target:
        move_ws = {"command": "STOP", "weight": 1.0}
        move_ad, turret_qe, turret_rf = vision_aim_commands(vision_target)
    elif gate_phase in {"post_align_delay", "judging", "confirmed"}:
        move_ws = {"command": "STOP", "weight": 1.0}
        move_ad = {"command": "", "weight": 0.0}
    ...
    # Near a target, lock the body and let only the turret do fine aiming.
    # Body rotation close to the target amplifies web/API latency and causes
    # the aim line to pass over the target repeatedly.
    if should_lock_body_near_lidar_target(lidar_target):
        move_ad = {"command": "", "weight": 0.0}
    if gate_phase in {"post_align_delay", "judging", "confirmed"}:
        move_ws = {"command": "STOP", "weight": 1.0}
        move_ad = {"command": "", "weight": 0.0}
    ...
    with state_lock:
        ...
        # Cooldown is tracked with monotonic time in a private helper key.
