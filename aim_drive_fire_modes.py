...
def set_mode_value(value: str) -> str:
    ...
    mode = "fire" if not DRIVE_MODE_ENABLED or requested in {"fire", "aim", "attack", "shoot"} else "drive"
    current_path = []
    current_path_index = 0
    last_mode_update = monotonic()
    return mode
...
def combined_get_action():
    ...
    latest_position = parse_position(data)
    latest_body_yaw = parse_body_yaw(data)
    if not DRIVE_MODE_ENABLED or mode == "fire":
...
def combined_lidar_view():
  ...
  <div style="margin:0 0 12px 0">
    <button disabled style="padding:8px 12px;border:1px solid #555;background:#222;color:#777;border-radius:6px">Drive mode disabled</button>
  </div>
"""
...
<body><h2>Drive / Fire Mode</h2><button disabled>Drive disabled</button><button onclick="setMode('fire')">Fire mode</button><canvas id="map" width="720" height="720"></canvas><pre id="s" style="display:none"></pre>
<script>
...
async function setDestFromEvent(e){const r=c.getBoundingClientRect();const dx=wx(e.clientX-r.left),dz=wz(e.clientY-r.top);await fetch('/set_destination',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({x:dx,y:0,z:dz})});await setMode('fire');}
