...
# Conservative LiDAR denoising. SOR is evaluated per distance band so sparse
# far returns are not unfairly compared with dense near returns.
STATISTICAL_OUTLIER_REMOVAL_ENABLED = False
SOR_MEAN_K = 8
SOR_STDDEV_MULTIPLIER = 2.5
SOR_RANGE_BAND_M = 15.0
SOR_MIN_EVALUATION_NEIGHBORS = 3

RADIUS_OUTLIER_REMOVAL_ENABLED = False
ROR_BASE_RADIUS_M = 0.65
ROR_RADIUS_PER_METER = 0.014
ROR_MAX_RADIUS_M = 2.40
ROR_MIN_NEIGHBORS = 2
OUTLIER_GRID_CELL_M = 2.50
...
# Destination driving is mode-separated from the existing aim/fire controller.
# Receiving a destination switches to drive mode; /mode can switch back to fire.
DRIVE_MODE_ENABLED = True
drive_mode = "fire"
drive_destination: dict[str, float] | None = None
drive_position = {"x": 60.0, "y": 10.0, "z": 27.23}
drive_body_yaw = 0.0
drive_obstacles: list[dict[str, Any]] = []
drive_active_obstacles: list[dict[str, Any]] = []
drive_path: list[tuple[float, float]] = []
drive_path_index = 0
drive_started = False
last_drive_action = make_action("STOP", 1.0)
last_drive_mode_update = monotonic()
...
def outlier_grid_key(point: dict[str, Any]) -> tuple[int, int, int]:
    position = point["position"]
    return (
        floor(float(position["x"]) / OUTLIER_GRID_CELL_M),
        floor(float(position["y"]) / OUTLIER_GRID_CELL_M),
        floor(float(position["z"]) / OUTLIER_GRID_CELL_M),
    )


def build_outlier_grid(points: list[dict[str, Any]]) -> dict[tuple[int, int, int], list[int]]:
    grid: dict[tuple[int, int, int], list[int]] = defaultdict(list)
    for index, point in enumerate(points):
        grid[outlier_grid_key(point)].append(index)
    return dict(grid)


def nearby_point_distances(
    ...
    position = point["position"]
    center = outlier_grid_key(point)
    maximum_distance_sq = maximum_distance * maximum_distance if maximum_distance is not None else None
    ...
    for dx in range(-search_cells, search_cells + 1):
        for dy in range(-search_cells, search_cells + 1):
            for dz in range(-search_cells, search_cells + 1):
                for other_index in grid.get((center[0] + dx, center[1] + dy, center[2] + dz), []):
                    if other_index == point_index:
                        continue
                    other = points[other_index]["position"]
                    distance_sq = (
                        (float(position["x"]) - float(other["x"])) ** 2
                        + (float(position["y"]) - float(other["y"])) ** 2
                        + (float(position["z"]) - float(other["z"])) ** 2
                    )
                    if maximum_distance_sq is not None and distance_sq > maximum_distance_sq:
                        continue
...
def statistical_outlier_removal(
    ...
    if not STATISTICAL_OUTLIER_REMOVAL_ENABLED or len(points) < 4:
        return points, 0
    grid = build_outlier_grid(points)
    mean_distances: list[float | None] = []
    range_bands: dict[int, list[float]] = defaultdict(list)
    search_cells = max(1, int(math.ceil(ROR_MAX_RADIUS_M / OUTLIER_GRID_CELL_M)) + 1)
    for index, point in enumerate(points):
        distances = nearby_point_distances(points, grid, index, search_cells)
        if len(distances) < SOR_MIN_EVALUATION_NEIGHBORS:
            mean_distances.append(None)
            continue
        selected = distances[:SOR_MEAN_K]
        mean_distance = sum(selected) / len(selected)
        mean_distances.append(mean_distance)
        range_bands[int(float(point["distance"]) // SOR_RANGE_BAND_M)].append(mean_distance)

    thresholds: dict[int, float] = {}
    for band, values in range_bands.items():
        if len(values) < 3:
            continue
        average = sum(values) / len(values)
        variance = sum((value - average) ** 2 for value in values) / len(values)
        thresholds[band] = average + SOR_STDDEV_MULTIPLIER * math.sqrt(variance)

    kept: list[dict[str, Any]] = []
    removed = 0
    for point, mean_distance in zip(points, mean_distances):
        threshold = thresholds.get(int(float(point["distance"]) // SOR_RANGE_BAND_M))
        if mean_distance is not None and threshold is not None and mean_distance > threshold:
            removed += 1
        else:
            kept.append(point)
    return kept, removed
...
def radius_outlier_removal(
    ...
    if not RADIUS_OUTLIER_REMOVAL_ENABLED or len(points) < 3:
        return points, 0
    grid = build_outlier_grid(points)
    kept: list[dict[str, Any]] = []
    removed = 0
    for index, point in enumerate(points):
        radius = min(
            ROR_MAX_RADIUS_M,
            ROR_BASE_RADIUS_M + float(point["distance"]) * ROR_RADIUS_PER_METER,
        )
        search_cells = max(1, int(math.ceil(radius / OUTLIER_GRID_CELL_M)))
        neighbors = nearby_point_distances(
            points,
            grid,
            index,
            search_cells,
        ...
        if len(neighbors) < ROR_MIN_NEIGHBORS:
            removed += 1
        else:
            kept.append(point)
    return kept, removed
...
def filter_lidar_outliers(
    rays: list[dict[str, Any]],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    raw_hits = detected_rays(rays)
    sor_hits, sor_removed = statistical_outlier_removal(raw_hits)
    filtered_hits, radius_removed = radius_outlier_removal(sor_hits)
    retained_ids = {id(point) for point in filtered_hits}
    filtered_rays = [
        point
        if not point.get("isDetected", False) or id(point) in retained_ids
        else {**point, "isDetected": False, "outlierRemoved": True}
        for point in rays
    ]
    return filtered_rays, filtered_hits, {
        "method": "statistical_then_radius_outlier_removal",
        "rawDetectedPointCount": len(raw_hits),
        "statisticalRemovedCount": sor_removed,
        "radiusRemovedCount": radius_removed,
        "filteredDetectedPointCount": len(filtered_hits),
        "totalRemovedCount": sor_removed + radius_removed,
        "statistical": {
            "enabled": STATISTICAL_OUTLIER_REMOVAL_ENABLED,
            "meanK": SOR_MEAN_K,
            "stddevMultiplier": SOR_STDDEV_MULTIPLIER,
            "rangeBandM": SOR_RANGE_BAND_M,
        },
        "radius": {
            "enabled": RADIUS_OUTLIER_REMOVAL_ENABLED,
            "baseRadiusM": ROR_BASE_RADIUS_M,
            "radiusPerMeter": ROR_RADIUS_PER_METER,
            "maxRadiusM": ROR_MAX_RADIUS_M,
            "minNeighbors": ROR_MIN_NEIGHBORS,
        },
    }
...
def summarize_lidar(data: dict[str, Any]) -> dict[str, Any]:
    raw_rays = parse_lidar_rays(data)
    rays, hits, outlier_filter = filter_lidar_outliers(raw_rays)
    ...
    return {
        ...
        "rawRayCount": len(raw_rays),
        "rawDetectedPointCount": outlier_filter["rawDetectedPointCount"],
        "filteredDetectedPointCount": len(hits),
...
# =============================================================================
# 7. Destination driving (A* + obstacle avoidance)
# =============================================================================
def drive_float(value: Any, default: float = 0.0) -> float:
    parsed = safe_float(value, default)
    return default if parsed is None else float(parsed)


def normalize_drive_obstacles(items: Any) -> list[dict[str, Any]]:
    cleaned: list[dict[str, Any]] = []
    for obstacle in items if isinstance(items, list) else []:
        if not isinstance(obstacle, dict):
            continue
        try:
            cleaned.append({
                "x_min": max(MAP_X_MIN, drive_float(obstacle["x_min"])),
                "x_max": min(MAP_X_MAX, drive_float(obstacle["x_max"])),
                "z_min": max(MAP_Z_MIN, drive_float(obstacle["z_min"])),
                "z_max": min(MAP_Z_MAX, drive_float(obstacle["z_max"])),
                "source": obstacle.get("source", "json"),
            })
        except KeyError:
            continue
    return cleaned
...
def load_drive_obstacles() -> list[dict[str, Any]]:
    if not OBSTACLE_CACHE_PATH.exists():
        return []
    try:
        payload = json.loads(OBSTACLE_CACHE_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return []
    return normalize_drive_obstacles(payload.get("obstacles", []) if isinstance(payload, dict) else [])


def save_drive_obstacles() -> None:
    try:
        OBSTACLE_CACHE_PATH.write_text(
            json.dumps({"obstacles": drive_obstacles}, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
    except OSError:
        pass
...
def sync_drive_pose(data: dict[str, Any]) -> None:
    global drive_position, drive_body_yaw
    position = latest_player_position_from_info(data)
    drive_position = {
        "x": drive_float(position.get("x"), drive_position["x"]),
        "y": drive_float(position.get("y"), drive_position["y"]),
        "z": drive_float(position.get("z"), drive_position["z"]),
    }
    yaw_keys = (
        "playerBodyX",
        "Player_Body_X",
    ...
    has_direct_yaw = any(key in data for key in yaw_keys)
    body = data.get("body") or data.get("playerBody") or data.get("player_body")
    has_nested_yaw = isinstance(body, dict) and any(key in body for key in ("x", "yaw", "y"))
    rotation = data.get("lidarRotation")
    has_lidar_yaw = isinstance(rotation, dict) and "y" in rotation
    if has_direct_yaw or has_nested_yaw or has_lidar_yaw:
        drive_body_yaw = body_yaw_from_info(data)


def drive_obstacle_margin(obstacle: dict[str, Any], base: float) -> float:
    width = max(0.0, drive_float(obstacle.get("x_max")) - drive_float(obstacle.get("x_min")))
    depth = max(0.0, drive_float(obstacle.get("z_max")) - drive_float(obstacle.get("z_min")))
    return base + min(OBSTACLE_MAX_EXTRA_MARGIN, max(width, depth) * OBSTACLE_SIZE_MARGIN_FACTOR)


def is_drive_obstacle_cell(gx: int, gz: int) -> bool:
    x, z = grid_to_world(gx, gz)
    if distance_2d(drive_position["x"], drive_position["z"], x, z) <= START_CLEAR_RADIUS:
        return False
    for obstacle in drive_active_obstacles or drive_obstacles:
        margin = drive_obstacle_margin(obstacle, OBSTACLE_CELL_MARGIN)
        if (
            obstacle["x_min"] - margin <= x <= obstacle["x_max"] + margin
            and obstacle["z_min"] - margin <= z <= obstacle["z_max"] + margin
        ):
            return True
    return False


def drive_heuristic(start: tuple[int, int], goal: tuple[int, int]) -> float:
    dx = abs(start[0] - goal[0])
    dz = abs(start[1] - goal[1])
    return (dx + dz) + (math.sqrt(2.0) - 2.0) * min(dx, dz)


def drive_astar(start: tuple[int, int], goal: tuple[int, int]) -> list[tuple[int, int]]:
    open_set = [(0.0, start)]
    ...
    while open_set:
        ...
        if current == goal:
            ...
            while walk in came_from:
                reversed_path.append(walk)
                walk = came_from[walk]
            return [start] + list(reversed(reversed_path))
        for dx, dz in directions:
            neighbor = (current[0] + dx, current[1] + dz)
            if not is_inside_map(*neighbor) or is_drive_obstacle_cell(*neighbor):
                continue
            if dx and dz and (
                is_drive_obstacle_cell(current[0] + dx, current[1])
                or is_drive_obstacle_cell(current[0], current[1] + dz)
            ):
                continue
            tentative = g_score[current] + (math.sqrt(2.0) if dx and dz else 1.0)
            if neighbor not in g_score or tentative < g_score[neighbor]:
                came_from[neighbor] = current
                g_score[neighbor] = tentative
                heapq.heappush(open_set, (tentative + drive_heuristic(neighbor, goal), neighbor))
    return []


def drive_point_blocked(x: float, z: float, margin: float = OBSTACLE_CLEAR_MARGIN) -> bool:
    return any(
        obstacle["x_min"] - margin <= x <= obstacle["x_max"] + margin
        and obstacle["z_min"] - margin <= z <= obstacle["z_max"] + margin
        for obstacle in drive_active_obstacles
    )


def drive_distance_to_obstacle(x: float, z: float, obstacle: dict[str, Any]) -> float:
    dx = max(obstacle["x_min"] - x, 0.0, x - obstacle["x_max"])
    dz = max(obstacle["z_min"] - z, 0.0, z - obstacle["z_max"])
    return math.hypot(dx, dz)


def nearest_drive_obstacle_distance(x: float, z: float) -> float | None:
    if not drive_active_obstacles:
        return None
    return min(drive_distance_to_obstacle(x, z, obstacle) for obstacle in drive_active_obstacles)


def drive_has_clear_path(ax: float, az: float, bx: float, bz: float) -> bool:
    if not drive_active_obstacles:
        return True
    ...
    for index in range(samples + 1):
        ...
        z = az + (bz - az) * ratio
        for obstacle in drive_active_obstacles:
            margin = drive_obstacle_margin(obstacle, OBSTACLE_CLEAR_MARGIN)
            if (
                obstacle["x_min"] - margin <= x <= obstacle["x_max"] + margin
                and obstacle["z_min"] - margin <= z <= obstacle["z_max"] + margin
            ):
...
def drive_toward(tx: float, tz: float) -> dict[str, Any]:
    error = normalize_angle(
        get_target_angle(drive_position["x"], drive_position["z"], tx, tz) - drive_body_yaw
    )
    distance = distance_2d(drive_position["x"], drive_position["z"], tx, tz)
    obstacle_distance = nearest_drive_obstacle_distance(drive_position["x"], drive_position["z"])
    ...
    if near_obstacle and abs(error) > 45:
        return make_action("W", 0.12, "D" if error > 0 else "A", 0.75)
    ...
    if abs(error) > 70:
        return make_action("W", 0.22, "D" if error > 0 else "A", 0.80)
    if abs(error) > 35:
...
def choose_drive_waypoint() -> tuple[float, float]:
    maximum = min(len(drive_path) - 1, drive_path_index + LOOKAHEAD_WAYPOINTS)
    minimum = min(len(drive_path) - 1, drive_path_index + 1)
    for index in range(maximum, minimum - 1, -1):
        waypoint = drive_path[index]
        if drive_has_clear_path(drive_position["x"], drive_position["z"], *waypoint):
            return waypoint
    return drive_path[drive_path_index]


def decide_drive_action() -> dict[str, Any]:
    global drive_active_obstacles, drive_path, drive_path_index
    if drive_destination is None:
        return make_action("STOP", 1.0)
    drive_active_obstacles = drive_obstacles
    px, pz = drive_position["x"], drive_position["z"]
    gx, gz = drive_destination["x"], drive_destination["z"]
    if drive_point_blocked(px, pz, OBSTACLE_ESCAPE_MARGIN):
        return make_action("S", 0.40)
    if distance_2d(px, pz, gx, gz) < GOAL_THRESHOLD:
        return make_action("STOP", 1.0)
    if drive_has_clear_path(px, pz, gx, gz):
        drive_path = []
        drive_path_index = 0
        return drive_toward(gx, gz)
    if drive_path and distance_2d(px, pz, *drive_path[0]) > REPLAN_DISTANCE:
        drive_path = []
        drive_path_index = 0
    if not drive_path:
        grid_path = drive_astar(world_to_grid(px, pz), world_to_grid(gx, gz))
        if not grid_path:
            return make_action("W", 0.16)
        drive_path = [grid_to_world(x, z) for x, z in grid_path]
        drive_path_index = 0
    search_end = min(len(drive_path), drive_path_index + 8)
    drive_path_index = max(
        drive_path_index,
        min(
            range(drive_path_index, search_end),
            key=lambda index: distance_2d(px, pz, *drive_path[index]),
        ),
    )
    while (
        drive_path_index < len(drive_path) - 1
        and distance_2d(px, pz, *drive_path[drive_path_index]) < WAYPOINT_THRESHOLD
    ):
        drive_path_index += 1
    return drive_toward(*choose_drive_waypoint())


def build_drive_preview_path() -> None:
    global drive_active_obstacles, drive_path, drive_path_index
    drive_active_obstacles = drive_obstacles
    drive_path = []
    drive_path_index = 0
    ...
    gx, gz = drive_destination["x"], drive_destination["z"]
    if drive_has_clear_path(px, pz, gx, gz):
        drive_path = [(px, pz), (gx, gz)]
        return
    grid_path = drive_astar(world_to_grid(px, pz), world_to_grid(gx, gz))
    drive_path = [grid_to_world(x, z) for x, z in grid_path]


def set_drive_mode(value: str) -> str:
    global drive_mode, drive_path, drive_path_index, drive_started, last_drive_mode_update
    requested = str(value).lower()
    drive_mode = "fire" if requested in {"fire", "aim", "attack", "shoot"} else "drive"
    if drive_mode == "fire":
        drive_started = False
    drive_path = []
    drive_path_index = 0
...
def drive_mode_payload() -> dict[str, Any]:
    return {
        "mode": drive_mode,
        "driveModeEnabled": DRIVE_MODE_ENABLED,
        "position": drive_position,
        "bodyYaw": drive_body_yaw,
        "destination": drive_destination,
        "obstacles": drive_obstacles,
        "path": [{"x": x, "z": z} for x, z in drive_path],
        "pathIndex": drive_path_index,
        "started": drive_started,
        "lastDriveAction": last_drive_action,
        "lastModeUpdate": last_drive_mode_update,
    }


drive_obstacles = load_drive_obstacles()
    ...
    return jsonify(
        {
            ...
            "filteredDetectedPointCount": scan.get("filteredDetectedPointCount", 0),
            "outlierFilter": scan.get("outlierFilter", {}),
...
<head>
  ...
  <style>
    ...
    .modebar { display: flex; align-items: center; gap: 8px; margin: 0 0 14px 0; }
    .modebar button { border: 1px solid #555; background: #222; color: #eee; padding: 9px 14px; border-radius: 6px; cursor: pointer; font-weight: bold; }
    .modebar button.active-drive { background: #176b3a; border-color: #38d47c; }
    .modebar button.active-fire { background: #8b2525; border-color: #ff6868; }
    .modebar span { color: #ddd; font-size: 14px; margin-left: 4px; }
  </style>
</head>
<body>
  <h2>LiDAR Auto Body Alignment v8.7: Tracking Mode Fix + Action Debug</h2>
  <div class="modebar">
    <button id="driveModeButton">주행 모드</button>
    <button id="fireModeButton">전투 모드</button>
    <span id="modeText">현재 모드 확인 중...</span>
  </div>
...
function draw(scan) {
  ...
  statusText.textContent=[
    ...
    'filteredDetectedPointCount: '+scan.filteredDetectedPointCount
      +' | SOR removed='+(scan.outlierFilter ? scan.outlierFilter.statisticalRemovedCount : 0)
      +' | Radius removed='+(scan.outlierFilter ? scan.outlierFilter.radiusRemovedCount : 0),
...
function updateModeControl(payload) {
  const mode = payload && payload.mode === 'drive' ? 'drive' : 'fire';
  driveModeButton.classList.toggle('active-drive', mode === 'drive');
  fireModeButton.classList.toggle('active-fire', mode === 'fire');
  modeText.textContent = mode === 'drive'
    ? '현재: 주행 모드'
    : '현재: 전투 모드';
}
async function setControlMode(mode) {
  driveModeButton.disabled = true;
  fireModeButton.disabled = true;
  try {
    const r = await fetch('/mode?mode=' + encodeURIComponent(mode), {cache:'no-store'});
    updateModeControl(await r.json());
    if (mode === 'drive') {
      location.href = '/mode_view';
    }
  } catch (e) {
    modeText.textContent = '모드 전환 실패: ' + String(e);
  } finally {
    driveModeButton.disabled = false;
    fireModeButton.disabled = false;
  }
}
async function refreshMode() {
  try {
    const r = await fetch('/mode_state', {cache:'no-store'});
    updateModeControl(await r.json());
  } catch (e) {
    modeText.textContent = '모드 상태 확인 실패';
  }
}
driveModeButton.addEventListener('click', () => setControlMode('drive'));
fireModeButton.addEventListener('click', () => setControlMode('fire'));
...
setInterval(refreshMode,500); refreshMode();
</script>
</body>
</html>
"""
...
@app.route("/mode_view", methods=["GET"])
def mode_view():
    return r"""
<!doctype html>
<html lang="ko">
<head>
  <meta charset="utf-8">
  <title>Tank 주행 지도</title>
  <style>
    body { margin: 18px; background: #101214; color: #eee; font-family: Arial, sans-serif; }
    .toolbar { display: flex; align-items: center; gap: 9px; margin-bottom: 12px; flex-wrap: wrap; }
    button { padding: 9px 14px; border-radius: 6px; border: 1px solid #555; background: #222; color: #eee; cursor: pointer; font-weight: bold; }
    #driveButton { background: #176b3a; border-color: #38d47c; }
    #startButton { background: #146c43; border-color: #40e08b; }
    #stopButton { background: #664b16; border-color: #e4b348; }
    #fireButtonMap { background: #642020; border-color: #d45858; }
    #map { background: #171a1d; border: 1px solid #555; cursor: crosshair; max-width: 92vw; height: auto; }
    #state { margin-top: 10px; color: #ddd; font-size: 14px; white-space: pre-wrap; }
    .hint { color: #ffd166; margin: 8px 0 12px; }
  </style>
</head>
<body>
  <h2>주행 지도</h2>
  <div class="toolbar">
    <button id="driveButton">주행 모드</button>
    <button id="startButton">출발</button>
    <button id="stopButton">정지</button>
    <button id="fireButtonMap">전투 모드로 복귀</button>
    <span id="modeLabel">상태 확인 중...</span>
  </div>
  <div class="hint">지도에서 목적지를 먼저 찍어 경로를 확인한 뒤, 출발 버튼을 누르세요.</div>
...
const MAP_SIZE = 300;
const PAD = 28;
function sx(value){ return PAD + Number(value || 0) / MAP_SIZE * (canvas.width - PAD * 2); }
function sy(value){ return canvas.height - PAD - Number(value || 0) / MAP_SIZE * (canvas.height - PAD * 2); }
function wx(pixel){ return Math.max(0, Math.min(MAP_SIZE, (pixel - PAD) / (canvas.width - PAD * 2) * MAP_SIZE)); }
function wz(pixel){ return Math.max(0, Math.min(MAP_SIZE, (canvas.height - PAD - pixel) / (canvas.height - PAD * 2) * MAP_SIZE)); }
function draw(data){
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.strokeStyle = '#34383d';
  ctx.fillStyle = '#aaa';
  ctx.font = '12px Arial';
  for(let value=0; value<=MAP_SIZE; value+=50){
    ctx.beginPath();
    ctx.moveTo(sx(value), sy(0)); ctx.lineTo(sx(value), sy(MAP_SIZE));
    ctx.moveTo(sx(0), sy(value)); ctx.lineTo(sx(MAP_SIZE), sy(value));
    ctx.stroke();
    ctx.fillText(String(value), sx(value)+3, sy(0)-5);
    if(value>0) ctx.fillText(String(value), sx(0)+4, sy(value)-4);
  }
  for(const obstacle of data.obstacles || []){
    ctx.fillStyle = 'rgba(255,75,75,.55)';
    ctx.fillRect(
      sx(obstacle.x_min), sy(obstacle.z_max),
      sx(obstacle.x_max)-sx(obstacle.x_min),
      sy(obstacle.z_min)-sy(obstacle.z_max)
    );
  }
  ...
  if(path.length){
    ...
    path.forEach((point,index) => {
      if(index===0) ctx.moveTo(sx(point.x),sy(point.z));
      else ctx.lineTo(sx(point.x),sy(point.z));
    });
    ctx.stroke();
    ctx.lineWidth = 1;
  }
  if(data.destination){
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.arc(sx(data.destination.x),sy(data.destination.z),9,0,Math.PI*2);
    ctx.stroke();
    ctx.lineWidth = 1;
  }
  const position = data.position || {};
  ctx.fillStyle = data.mode === 'drive' ? '#44d66b' : '#ff5555';
  ctx.beginPath();
  ctx.arc(sx(position.x),sy(position.z),8,0,Math.PI*2);
  ctx.fill();
  const yaw = Number(data.bodyYaw || 0) * Math.PI / 180;
  ctx.strokeStyle = '#fff';
  ctx.beginPath();
  ctx.moveTo(sx(position.x),sy(position.z));
  ctx.lineTo(sx(position.x)+Math.sin(yaw)*22,sy(position.z)-Math.cos(yaw)*22);
  ctx.stroke();
  modeLabel.textContent = data.mode === 'drive' ? '현재: 주행 모드' : '현재: 전투 모드';
  if(data.mode === 'drive') {
    modeLabel.textContent += data.started ? ' / 주행 중' : ' / 출발 대기';
  }
  stateText.textContent =
    '현재 위치: X ' + Number(position.x || 0).toFixed(1) + ' / Z ' + Number(position.z || 0).toFixed(1)
    + '\n목적지: ' + (data.destination ? 'X '+Number(data.destination.x).toFixed(1)+' / Z '+Number(data.destination.z).toFixed(1) : '없음')
    + '\n경로: ' + Number(data.pathIndex || 0) + ' / ' + path.length
    + '\n명령: ' + JSON.stringify(data.lastDriveAction || {});
}
async function refreshMap(){
  try {
    const response = await fetch('/mode_state', {cache:'no-store'});
    draw(await response.json());
  } catch(error) {
    stateText.textContent = '지도 상태 수신 실패: ' + String(error);
  }
}
canvas.addEventListener('click', async event => {
  ...
  const x = wx((event.clientX - rect.left) * scaleX);
  const z = wz((event.clientY - rect.top) * scaleY);
  await fetch('/set_destination', {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({x, y:0, z})
  });
  await refreshMap();
});
document.getElementById('driveButton').addEventListener('click', async () => {
  await fetch('/mode?mode=drive', {cache:'no-store'});
  await refreshMap();
});
document.getElementById('startButton').addEventListener('click', async () => {
  const response = await fetch('/drive_start', {method:'POST'});
  const payload = await response.json();
  if(!response.ok) {
    stateText.textContent = payload.message || '목적지를 먼저 지정하세요.';
    return;
  }
  draw(payload);
});
document.getElementById('stopButton').addEventListener('click', async () => {
  await fetch('/drive_stop', {method:'POST'});
  await refreshMap();
});
document.getElementById('fireButtonMap').addEventListener('click', async () => {
  await fetch('/mode?mode=fire', {cache:'no-store'});
  location.href = '/lidar_view';
});
setInterval(refreshMap, 300);
refreshMap();
</script>
</body>
</html>
...
def get_action():
    ...
    """
    global last_drive_action
    request_body = request.get_json(silent=True) or {}
    if isinstance(request_body, dict):
        sync_drive_pose(request_body)

    if DRIVE_MODE_ENABLED and drive_mode == "drive":
        last_drive_action = decide_drive_action() if drive_started else make_action("STOP", 1.0)
        response_body = {
            **last_drive_action,
        ...
        with state_lock:
            ...
            action_debug_state["driveMode"] = drive_mode_payload()
...
def mode_route():
    if request.method == "POST":
        data = request.get_json(silent=True) or {}
        set_drive_mode(data.get("mode", request.args.get("mode", drive_mode)))
    elif request.args.get("mode"):
        set_drive_mode(request.args["mode"])
    return jsonify(drive_mode_payload())


@app.route("/mode_state", methods=["GET"])
def mode_state():
    return jsonify(drive_mode_payload())
...
def set_destination():
    global drive_destination, drive_path, drive_path_index, drive_started
    data = request.get_json(force=True, silent=True) or {}
    try:
        if "destination" in data:
            ...
            x = float(data["x"])
            y = drive_float(data.get("y"), 0.0)
            z = float(data["z"])
    except (KeyError, TypeError, ValueError):
        return jsonify({"status": "ERROR", "message": "Invalid destination"}), 400
    drive_destination = {"x": x, "y": y, "z": z}
    set_drive_mode("drive")
    drive_started = False
    build_drive_preview_path()
    return jsonify({
        "status": "OK",
        "destination": drive_destination,
        "mode": drive_mode,
        "started": drive_started,
        "pathLength": len(drive_path),
    })


@app.route("/drive_start", methods=["POST", "GET"])
def drive_start():
    global drive_started, drive_path_index
    if drive_destination is None:
        return jsonify({"status": "ERROR", "message": "Destination is not set"}), 400
    set_drive_mode("drive")
    drive_started = True
    drive_path_index = 0
    build_drive_preview_path()
    return jsonify({"status": "OK", **drive_mode_payload()})


@app.route("/drive_stop", methods=["POST", "GET"])
def drive_stop():
    global drive_started, last_drive_action
    drive_started = False
    last_drive_action = make_action("STOP", 1.0)
    return jsonify({"status": "OK", **drive_mode_payload()})
...
def update_obstacle():
    global drive_obstacles, drive_path, drive_path_index
    data = request.get_json(force=True, silent=True) or {}
    obstacle_items = data.get("obstacles") if isinstance(data, dict) else None
    if isinstance(obstacle_items, list):
        drive_obstacles = normalize_drive_obstacles(obstacle_items)
        drive_path = []
        drive_path_index = 0
        save_drive_obstacles()
        return jsonify({
            "status": "OK",
            "message": "Navigation obstacle data received",
            "obstacleCount": len(drive_obstacles),
            "mode": drive_mode,
        })
    ...
    return jsonify({"status": "success", "message": "Obstacle data received", "obstacleCount": len(drive_obstacles)})
...
def init():
    global drive_obstacles, drive_path, drive_path_index, drive_started
    with state_lock:
        reset_runtime_state()
    set_drive_mode("fire")
    drive_obstacles = load_drive_obstacles()
...
if __name__ == "__main__":
    ...
    print("Drive mode/status: http://127.0.0.1:5000/mode_state")
    print("Switch to fire: http://127.0.0.1:5000/mode?mode=fire")
